---
name: ascii
description: Create professional ASCII diagrams for system architecture and component interactions. Use when the user requests a high-level overview of system flows, API sequences, or architectural component interactions in a plain-text format.
---

# SA ASCII Diagrams

This skill provides a standardised way to draw high-signal, professional ASCII diagrams that mimic Solution Architecture (SA) interaction diagrams.
...
3. **Directional Flow**: Use clear arrows (`--->`, `<---`, `<==>`) to indicate the direction of data or control flow.
...
2. **Define the Trigger**: Start the diagram with the user or system event that initiates the flow.
3. **Trace the Path**: Follow the primary "happy path" first, then add critical error branches if necessary.
